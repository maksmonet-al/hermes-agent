...
export default function FilesPage() {
  ...
  const createDirectory = async () => {
    ...
    if (!name) {
      showToast("Укажите имя папки", "error");
      return;
    }
    setCreating(true);
    try {
  ...
  return (
    <div className="flex min-w-0 max-w-full flex-col gap-4">
        ...
        <span className="flex min-w-0 items-center gap-3">
          ...
          <span className="min-w-0">
            <span className="block text-sm font-semibold uppercase tracking-[0.08em] text-foreground">
              {uploading ? "Загрузка" : draggingFiles ? "Отпустите для загрузки" : "Перетащите файлы сюда"}
            </span>
      ...
      >
        <DialogContent className="max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              Расположение: {activePath || "Загрузка"}
            </DialogDescription>
          </DialogHeader>
          <div className="p-4">
            <Input
      ...
      <DeleteConfirmDialog
        ...
        title={pendingDelete ? `Удалить ${pendingDelete.name}?` : "Удалить объект?"}
        description={
